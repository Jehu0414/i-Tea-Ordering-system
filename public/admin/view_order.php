...
// Fetch ordered items
// ✅ Adjust column name if needed (use your actual schema)
$stmt = $pdo->prepare("
  SELECT oi.*, i.name
  FROM order_items oi
...
// Take-out setup
$takeoutFeePerBox = 5;
$isTakeout = (strtolower($order['order_type'] ?? '') === 'take-out');
$total = 0;
$totalTakeoutBoxes = 0;
?>
<!doctype html>
...
<head>
  ...
  <style>
    ...
    .receipt h5 { font-weight: bold; margin-bottom: 10px; text-align: center; color: #2f3b2f; }
    .receipt table th, .receipt table td { padding: 6px; font-size: 14px; }
    .table-success th { background-color: #8a9a5b !important; color: #fff; }
    .btn-sage { background-color: #8a9a5b; color: #fff; border: none; }
    .btn-sage:hover { background-color: #7c8e54; }
  </style>
</head>
<body>
<div class="container py-4">
  <div class="receipt">
    ...
    <div><strong>Staff/Admin:</strong> <?= e($order['staff_name'] ?? 'N/A') ?> (<?= e($order['staff_role'] ?? '-') ?>)</div>
    ...
    <table class="table table-sm">
      ...
      <tbody>
        <?php if (!empty($items)): ?>
          <?php foreach($items as $it): ?>
            <?php
              ...
              $total += $lineTotal;
              if ($isTakeout) $totalTakeoutBoxes += $it['qty'];
            ?>
            <tr>
              <td><?= e($it['name']) ?></td>
        ...
        <?php if ($isTakeout && $totalTakeoutBoxes > 0): ?>
          <?php 
            $takeoutCost = $totalTakeoutBoxes * $takeoutFeePerBox; 
            $total += $takeoutCost; 
          ?>
          <tr>
    ...
    <hr>
    <p class="text-end fw-bold mb-1">Total: ₱<?= number_format($total, 2) ?></p>

    <?php if (!empty($order['amount_paid'])): ?>
      <p class="text-end mb-1"><strong>Amount Paid:</strong> ₱<?= number_format($order['amount_paid'], 2) ?></p>
      <p class="text-end mb-1"><strong>Change:</strong> ₱<?= number_format($order['change_due'], 2) ?></p>
    <?php endif; ?>
    ...
    <div class="no-print text-center mt-4">
      <button class="btn btn-sage btn-sm" onclick="window.print()">🖨 Print Receipt</button>
